...
DATA_DIR="/data/server/fullstack-template/data"
# 主库文件名(留空则 sqlite_derive 按 ENV 推导,与 app 代码定死的文件名一致:
# prod=template.prod.sqlite / dev=template.dev.sqlite;--db 显式覆盖)。
...
sqlite_derive() {
  ...
  #   prod → template.prod.sqlite / dev → template.dev.sqlite(--env 覆盖 ENV 后此处同步生效)。
  if [[ -z "$DB_NAME" ]]; then
    if [[ "$ENV" == "prod" ]]; then DB_NAME="template.prod.sqlite";
    else DB_NAME="template.dev.sqlite"; fi
  fi
  # TAG = DB 文件名去扩展名,用作备份目录名前缀(<tag>.<ts>)与轮转/回退 glob。
  # DB_NAME=template.prod.sqlite → TAG=template.prod;DB_NAME=prod.sqlite → TAG=prod。
  TAG="${DB_NAME%.*}"
