...
// 文件名由程序按 NODE_ENV 定死为 g-fullstack.<env>.sqlite(prod=g-fullstack.prod.sqlite /
// dev=g-fullstack.dev.sqlite),不外配;未设 DB_DIR 则用 yaml 的 dir(相对 data/ → WORKDIR /app)。
const isProd = env === 'prod';
const dbFilename = isProd ? 'g-fullstack.prod.sqlite' : 'g-fullstack.dev.sqlite';
