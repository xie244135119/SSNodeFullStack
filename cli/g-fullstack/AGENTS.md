...
| `pnpm --filter g-fullstack-web dev` / `pnpm --filter g-fullstack-backend dev` | 单起一端 |
| `pnpm --filter g-fullstack-backend migration:generate src/database/migrations/<name>` | 新增实体后生成迁移(必须,见硬约束 ①) |
...
8. **DB 文件名程序定死**:`g-fullstack.prod.sqlite`/`g-fullstack.dev.sqlite`,改名要同步 `sqlite.config.ts`、`configuration.ts`、`ops/sqlite/config.sh`、ops 探针四处。
