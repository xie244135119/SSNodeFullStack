...
- `web/package.json`:`name`、`description`(同时影响 `pnpm --filter g-fullstack-web` 与 `backend/scripts/build.cjs` 的 `safeName`)
...
- 新增实体后 `pnpm --filter g-fullstack-backend migration:generate src/database/migrations/xxxx`。
- WAL + PRAGMA 加固:`journal_mode=WAL / synchronous=NORMAL / busy_timeout=5000 / foreign_keys=ON`。
- DB 文件名程序定死:`g-fullstack.prod.sqlite`/`g-fullstack.dev.sqlite`(改名要同步 `sqlite.config.ts`、`configuration.ts`、`ops/sqlite/config.sh`、ops 探针)。
...
- `pnpm --filter g-fullstack-backend build` = webpack 单文件打包 `src/main.ts` → `dist/main.js`,自动 bump version。
- `pnpm --filter g-fullstack-backend buildops` = build 之后组装运维包到 `backend/release/`。
