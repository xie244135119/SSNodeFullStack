...
export class SqliteBackupProbe implements OpsProbe {
  ...
  async collect(): Promise<OpsProbeResult> {
    ...
    try {
      entries = fs
        ...
        .filter((d) => d.isDirectory() && d.name.startsWith(`template.${env}.`))
        .map((d) => d.name)
        .sort()
        .reverse();
    } catch (e) {
  ...
  /** 读备份目录里三件套构成描述（如 template.prod.sqlite + -wal + -shm） */
  private readTriplet(dir: string): string {
    const env = process.env.NODE_ENV === 'prod' ? 'prod' : 'develop';
    const base = `template.${env}.sqlite`;
  ...
  private formatTs(dirName: string): string {
    // dirName 形如 template.prod.20260812-104311 → 2026-08-12 10:43:11
